...
void initcontext(struct context_t * ctx, const int fd, const int size) {
    ...
    ctx->cb_w.aio_sigevent.sigev_notify = SIGEV_SIGNAL;
    ctx->cb_w.aio_sigevent.sigev_signo = SIGUSR1;
...
void sigusr1Handler(int sig, siginfo_t *si, void *ucontext) {
    if (si->si_code != SI_ASYNCIO) return; // What's!?
    struct context_t * ctx = (struct context_t *)si->si_value.sival_ptr;
...
void run(char * filename, int filesize) {
    ...
    FAILCHECK(sigaction(SIGUSR1, &sa, NULL) == -1, "sigaction")
